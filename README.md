...
뜨고 BepInEx 로그에도 남는다. **멀티 세션에서는 기본으로 잠긴다.** 설정에서 열 수 있지만
실험이고(`멀티 자동 배치(실험)`), 켜도 **호스트에서만** 동작한다 - 쓰기가 서버 API 라 클라이언트로
접속한 세션에서는 애초에 돌지 않는다. 개발사가 금지한 것은 아니고 인벤토리 동기화 구현을 바꾸는
중이라 잠가 두는 편이 안전하다고 답했으므로 기본을 꺼 두었다([docs/LEGAL.md](docs/LEGAL.md)).
공식 FAQ 가 멀티 모드를 "참가자 전원 동의" 조건으로 두므로, 켤 때는 같이 하는 사람의 동의를 받는다.
...
- **쓰기는 자동 배치 하나뿐이다.** 그 밖에는 게임 상태를 읽기만 한다. 자동 배치는 싱글플레이에서만
  동작하고 멀티에서는 기본으로 잠긴다(설정에서 여는 실험 옵션이 있고, 켜도 호스트 한정이다).
