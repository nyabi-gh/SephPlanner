...
뜨고 BepInEx 로그에도 남는다. **멀티 세션에서는 예외 없이 잠긴다.** 여는 옵션은 없다. 개발사가
인벤토리 동기화 구현을 바꾸는 중이라 잠가 두라고 답했기 때문이다([docs/LEGAL.md](docs/LEGAL.md)).
...
- **쓰기는 자동 배치 하나뿐이다.** 그 밖에는 게임 상태를 읽기만 한다. 자동 배치는 싱글플레이에서만
  동작하고 멀티에서는 예외 없이 잠긴다. 싱글 자동 배치는 개발사에게 허락받았다. 근거와 경위는
