...
뜨고 BepInEx 로그에도 남는다. **멀티 세션에서는 기본적으로 잠긴다.** `F3`의 "멀티 자동 배치(실험)"
을 명시적으로 켠 호스트만 잠금을 풀 수 있고, 클라이언트에서는 켜도 적용하지 않는다. 이 경로는
동기화 실기 검증이 끝나지 않았으므로 참가자 모두가 동의한 사설 방에서만 시험한다
...
켜고 끄기, 모서리, 크기, 폭, 불투명도, 후보 추천, 단축키와 멀티 자동 배치 실험 옵션이 거기 있다.
...
- **쓰기는 자동 배치 하나뿐이다.** 그 밖에는 게임 상태를 읽기만 한다. 자동 배치는 기본적으로
  싱글플레이에서만 동작한다. 멀티에서는 기본 잠금이며, 명시적 실험 옵션을 켠 호스트만 참가자
  전원이 동의한 사설 방에서 시험할 수 있다. 근거와 경위는 [docs/LEGAL.md](docs/LEGAL.md) 참고.
