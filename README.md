...
- `전투`: 천칭의 자동·불 우선(왼쪽)·얼음 우선(오른쪽), 단일·다수 대상과 추가 대상 적중 비율, 시간·자원 프리셋, 공격 순서·시간·실측 간격 보정, 활성 보호와 미지원 변경 허용.
- `마법`: 사용 여부와 시전 우선순위. 모든 마법이 마나를 공유한다.
- `DPS 내역`: 전체·초반·후반·빈 자원 시작 DPS, 공격별 피해, 천칭의 현재·추천 위치 효과, 미반영 효과와 자동 적용 제한 사유. 줄을 누르면 전체 설명을 본다.
