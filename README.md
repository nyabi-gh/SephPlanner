...
전용 전체화면에서도 보이고, 글꼴과 판때기를 게임에서 빌려 쓴다. `F7`로 접고 펴며 `F8`이 자동
배치다. 마우스와 키보드를 하나도 가져가지 않으므로 누를 것이 없고 조작은 전부 단축키다.
BepInEx 설정의 `NativePanel` 절에서 위치·폭·단축키를 바꾸고 통째로 끌 수도 있다. 배경과 한계는
[docs/RESEARCH.md](docs/RESEARCH.md) 의 "인게임 UI" 절 참고.
