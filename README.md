...
컨트롤 스택에도 들어가지 않는다. 그래서 누를 것이 없고 조작은 전부 단축키다.
...
**설정은 게임 설정 창의 "플래너" 탭에서 바꾼다.** 게임의 탭과 설정 한 줄을 복제해 붙인 것이라
생김새와 조작이 게임 그대로다. 켜고 끄기, 모서리, 크기, 폭, 불투명도, 후보 추천, 위 단축키 넷이
거기 있다. **이 탭은 입력을 받아도 된다** — HUD 의 무입력 원칙은 플레이 중 조작을 방해하지 않기
위한 것인데, 설정 창은 플레이어가 일부러 연 게임 자신의 창이기 때문이다.

같은 값이 BepInEx 설정의 `NativePanel` 절에도 있다. 어느 쪽으로 고쳐도 같은 자리를 읽고 쓴다.
배경과 한계는 [docs/RESEARCH.md](docs/RESEARCH.md) 의 "인게임 UI" 절 참고.
