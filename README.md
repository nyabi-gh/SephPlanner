...
마지막 항목이 따로 필요한 이유가 있다. CI 는 `SephPlanner.Core` 와 테스트만 돌린다 - 플러그인은
...
격자가 꽉 차 임시로 비켜둘 칸이 없으면 실행할 수 없는 수동 순서를 보여 주지 않고 자동 배치만
안내한다.
...
물러선다. 회전 가능 여부까지 쓰기 전에 확인하고, 적용 도중 실패하면 이미 옮긴 걸음을 역순으로 되돌린다. 결과와 이유는 화면에 한 줄로
뜨고 BepInEx 로그에도 남는다.
...
호스트보다 몇 초 더 걸리고, 확인이 안 되면 거기서 멈춰 되돌린다. **도는 동안에는 몇 번째
...
배치를 풀어 두므로 계산은 늘지 않는다. 무엇을 보고 있는지는 안내 줄에 늘 적힌다 — 미리보기
격자를 지금 배치로 착각하면 안 되기 때문이다. 후보를 가리키는 것은 번호가 아니라 후보의
...
- **프리셋 코드.** 게임에서 커스텀 로드아웃 코드를 복사한 뒤 "빌드 코드 붙여넣기"를 누르면
  ...
  살 수 있는 것 다음 순서), 과일 꼬치의 양수 카테고리는 아래 콤보 지정을
  대신 눌러 준다.
