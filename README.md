...
세피리아(Sephiria)용 비공식 컴패니언. 현재 인벤토리를 읽어 석판 배치를 최적화하고, 지금
...
쓴다. 방화벽 팝업이 뜨지 않고, 포트가 충돌하지 않으며, 같은 사용자 세션 밖에서는 열 수 없다.
...
오버레이는 .NET 런타임 없이 도는 자체 포함 exe로, 플러그인은 DLL 두 개로 묶인다(오버레이를
걷어내면 DLL 두 개만 남는다). BepInEx와 게임 파일은 넣지 않는다. 버전은 `Directory.Build.props`의 `<Version>`에서 나온다.
...
**`F8` 은 그 배치를 게임에 그대로 적용한다.** 게임이 수동 드래그에 쓰는 이동 경로를 그대로
타므로 손으로 옮긴 것과 결과가 같다. 계산 이후 인벤토리가 바뀌었으면 아무것도 건드리지 않고
물러서고, 적용 도중 실패하면 이미 옮긴 걸음을 역순으로 되돌린다. 결과와 이유는 화면에 한 줄로
뜨고 BepInEx 로그에도 남는다. **멀티 세션에서는 잠긴다**([docs/LEGAL.md](docs/LEGAL.md)).
...
켜고 끄기, 모서리, 크기, 폭, 불투명도, 후보 추천, 단축키가 거기 있다. 같은 값이 BepInEx 설정의
`NativePanel` 절에도 있어 어느 쪽으로 고쳐도 같은 자리를 읽고 쓴다.
...
WPF 로 된 별도 창이다. **인게임 화면이 생기기 전의 표시 수단이고, 이제 여기에만 있는 기능은
없다.** 실물 확인이 끝나면 배포물에서 빼며, 그때 zip 이 59MB 에서 200KB 로 줄고 서명 없는 exe
안내와 아이콘 덤프 의무도 함께 없어진다([docs/ROADMAP.md](docs/ROADMAP.md)).
