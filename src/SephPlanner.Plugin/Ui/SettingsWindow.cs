...
namespace SephPlanner.Plugin.Ui
{
    ...
    /// <summary>
    /// 우리가 직접 그리는 설정 창. 단축키로 열고 닫는다.
    ///
    /// <b>게임 설정 창에 탭으로 붙이는 길은 접었다.</b> 그 창은 탭 다섯 개에 딱 맞게 짜여 있어
    /// (줄 폭 382 = 74x5 + 3x4) 여섯째가 들어갈 자리가 없고, 선택 표시인 흰 돌기가 탭 그림마다
    /// 박혀 있어 버튼을 옮기면 게임 탭들이 저마다 어긋난다. 남는 자리에 욱여넣으면 우리 탭만
    /// 좁아져 이름이 잘린다. 우리 창은 폭도 줄 수도 우리가 정하므로 그 한계가 없다.
    ///
    /// <b>이 창은 입력을 받는다.</b> HUD 화면(<see cref="NativeHud"/>)이 입력을 하나도 가져가지
    /// 않는 것은 플레이 중 게임 조작을 방해하지 않기 위해서인데, 이 창은 플레이어가 일부러 연
    /// 것이라 그 이유가 걸리지 않는다. 대신 <see cref="SettingsPanel"/>이 게임의 <c>UIBase</c>를
    ...
    internal sealed class SettingsWindow
    {
        ...
        public bool IsAlive => _panel != null;
        ...
        public void Toggle(IList<OptionRow> rows, string hint)
        {
            if (!TryCreate(rows)) return;
        ...
        private bool TryCreate(IList<OptionRow> rows)
        {
            ...
            _base = _skin.BaseSize;
            Build(root, rows);
            Origin = _skin.Origin + $" 줄 {_rows.Count}개";
            return true;
        }

        private float S(float ratio) => _base * ratio;

        private void Build(UIRoot root, IList<OptionRow> rows)
        {
            var frame = Widgets.Fill("SephPlannerSettings", root.transform, NativeSkin.Frame);
            ...
            rect.sizeDelta = new Vector2(S(30f), 0f);
            ...
            foreach (var row in rows)
            {
                ...
                _rows.Add(new Row(content, _skin, _base, row, Refresh));
            }

            // 컨트롤러로 열었을 때 어디에도 초점이 없으면 화살표를 누를 방법이 없다.
            if (_rows.Count > 0) _panel.defaultSelectable = _rows[0].First;
        }

        private void BuildHeader(RectTransform parent)
        {
            var title = Widgets.Label("Title", parent, _skin, S(1.1f), NativeSkin.TextBright);
            title.text = "SephPlanner 설정";
