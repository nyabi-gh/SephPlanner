...
namespace SephPlanner.Core.Solver
{
    public sealed class CharmSlot
    {
        ...
        /// <b>게임이 실제로 계산하는 값(이웃이 세는 레벨, 마법서 지원량)에는 쓰지 않는다</b> -
    ...
    public sealed class SolverOptions
    {
        ...
        ///
        /// <b>켜지면 그 자리에서 <see cref="OperationCanceledException"/> 이 날아간다.</b> 예전에는
        /// 검사마다 중간까지 푼 것을 돌려주었는데, 그 부분 결과가 - 석판을 몇 장만 놓은 빔, 첫
        /// 배치만 채점한 결과 - <see cref="LayoutCache"/> 에 그대로 들어갈 수 있었다. 캐시가 한
        /// 번의 계획보다 오래 살면 그 뒤의 모든 계획이 그것을 최선이라고 말하게 된다. 입구마다
        /// "취소면 캐시하지 않는다" 를 기억하는 대신 부분 결과라는 것을 없앴다.
        ///
        /// 부르는 쪽이 통째로 버릴 때만 켜는 신호다.
        /// </summary>
