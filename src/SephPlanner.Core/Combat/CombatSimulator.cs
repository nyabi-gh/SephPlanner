...
namespace SephPlanner.Core.Combat
{
    public static class CombatSimulator
    {
            ...
            bool details = true, CancellationToken cancellation = default)
        {
            ...
            var mana = maxMana * scenario.InitialManaFraction;
            ...
            while (time < scenario.DurationSeconds)
            {
                ...
                mana = Math.Min(maxMana, mana + Math.Max(0, time - Math.Max(previous, regenReady)) * regeneration);
                previous = time;
                ...
                foreach (var state in automatic)
                    if (state.Next <= time)
                    {
                        ...
                        state.Next = time + state.Cooldown;
                    }
                if (actorReady <= time)
                {
                    var cast = magicReady <= time ? magic.FirstOrDefault(state => state.Charges > 0 && (infinite || state.Cost <= mana)) : null;
                    if (cast != null)
                    {
                        UseCharge(cast, time);
                        if (!infinite) mana -= cast.Cost;
                        regenReady = time + recoveryDelay;
                        Hit(cast, time, cast.Cost, true);
                        actorReady = time + Duration(cast.Item.Attack, stats, timing);
                        magicReady = time + Math.Max(0, snapshot.GlobalMagicCooldown);
                    }
                    else if (sequence.Count > 0)
                    {
                        ...
                        var paid = action.Cost > 0 && (infinite || mana >= action.Cost);
                        if (paid)
                        {
                            if (!infinite) mana -= action.Cost;
                            regenReady = time + recoveryDelay;
                        }
                        ...
                        actorReady = time + Duration(action.Item.Attack, stats, timing);
                    }
                ...
                if (magic.Count > 0 && sequence.Count == 0)
                {
                    ...
                    {
                        var affordable = state.Cost <= mana || infinite ? time : regeneration > 0 && state.Cost <= maxMana ?
                            Math.Max(time, regenReady) + (state.Cost - mana) / regeneration : double.PositiveInfinity;
            ...
            result.RemainingMana = Math.Min(maxMana, mana + Math.Max(0, scenario.DurationSeconds - Math.Max(previous, regenReady)) * regeneration);
            ...
            void Hit(ActionState state, double now, int cost, bool callbacks)
            {
                ...
                foreach (var effect in triggered)
                {
                    ...
                    effect.TriggerReady = now + effect.Item.Attack.TriggerCooldownSeconds;
        ...
        private static void UseCharge(ActionState state, double time)
        {
            ...
            state.Recharge.Enqueue(start + state.Cooldown);
        }
