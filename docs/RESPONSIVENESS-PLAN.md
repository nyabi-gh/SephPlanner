...
- **무엇을.** `GameReader.ReadMixer` 가 `MixerState` 에 `Near`(플레이어와의 거리가 문턱 안)를
  ...
  `TabletMixAdvisor.Rank` 를 돌린다. 표시는 `8ef8d42` 의 창 기준 그대로.
...
- **결과(2026-09-12 밤).** 스페이스는 더 이상 보내지 않는다(사용자 확인). Enter 로 실제 보낸
  기록은 아직 없다 - 아래 "인게임 확인" 절.

## 6. 등록부 대조 걷어내기 (인계 문서 ②) (완료 2026-09-12)

`b180312` 는 오늘 19:06 에 재시작한 게임에서 돌고 있다(로그 형식이 새 것). 첫 5분 집계는
...
**그 덤프가 왔다 - 제보 `09b4a20d`(옛 빌드 `b180312`, 20:21).** 75분 세션(폴링 17,944회),
...
## 6b. 등록부를 한 번만 훑기 (조건부, 아직 안 함)

6번 뒤 폴링에 남는 가장 큰 것은 **등록부 순회 세 번**이다 - 제보 `09b4a20d` 에서 합성기 0.25ms
+ 상자 0.15ms + 세피라이트 0.03ms 로, 남은 폴링 평균 ~0.55ms 의 여덟 할이다. 셋이 같은
`spawned` 사전을 따로 돈다.

한 번 돌며 셋을 한꺼번에 담으면 0.43 → 0.15ms 쯤이 된다. **안 하는 이유**는 그 0.28ms 가
60fps 한 프레임(16.7ms)의 1.7% 라 체감되지 않고, `All<T>` 의 단순함(부르는 쪽이 무엇을 찾는지만
말하면 된다)을 잃기 때문이다. **다시 열 조건**: 폴링 평균이 1ms 를 넘거나, 인벤토리가 많은 판에서
`등록부 훑기` 최악값이 프레임 예산을 먹는 제보가 오면.
...
## 인게임 확인 (2026-09-12 밤, 새 빌드 `8a53fad`)

제보 일곱(`bb869357`·`5c1dda61`·`44af17f8`·`cbf7fc5c`·`f82bd1d2`·`120a57ef`·`6fd678ef`),
47분 한 세션, **오류·경고 0줄**, F8 24회 성공. 판이 아티팩트 5 → 20, 석판 2 → 12 로 커졌다.

**1번 - 섰다.** 일곱 번 모두 `밀린 세대 - 지금 0`(세션 최악 1~2). 조언 풀이 **128회**가 배치
풀이 **78회**보다 50회 많다 - 그 50회가 전에는 판을 통째로 다시 풀던 것이다. 버린 풀이는
배치 1·조언 2 뿐이다(전에는 `게시 199 / 요청 204` 로 어느 것도 끝을 못 봤다).

**3번 - 섰다.** 일곱 중 여섯에서 `요청에서 게시까지 최근` 이 **0.2~0.5 ms** 다. 같은 세션의
배치 풀이가 평균 386 ms·최악 2,677 ms 인 판에서 게시가 0.3 ms 라는 것은 탐색을 건너뛴 그 길
뿐이다. 예외 하나(`cbf7fc5c` 480 ms)는 F8 이 아니라 평범한 재계산 뒤에 누른 것이다.

**5번 - 섰다.** 사용자 확인: "F10 누르고 스페이스 눌러도 전송 안 되고 정상 동작함." (일곱
자료의 `Note` 가 모두 `null` 이라 **Enter 로 실제 전송한 기록은 없다.** 안 되는 날이 오면
보내기 단추는 그대로 있고, 그때 Enter 를 직접 읽어 더한다.)

**6번 - 섰다.** `등록부 대조` 줄이 사라졌고 폴링 평균 **0.81 → 0.70 ms**. 예상(0.55)보다 덜
내려간 것은 판이 커져 `지문 계산과 제출` 이 0.27 → 0.32 ms 로 오른 몫이다.

**2a - 못 봤다.** 일곱 판 전부 `Mixer = null` 로 합성기가 있는 층이 없었다. 합성기 층에서 F10
한 번이 필요하다.

**4번 - 인게임 재현 불가.** 그 시나리오 퀘스트가 지나가 이 저장에서는 증표를 다시 들 수 없다
(사용자 확인). 근거는 제보 `6961d1a0`(증표가 실제로 있는 판) 로 계획을 지어 본 것과 단위
테스트뿐이다 - 적용기까지 실기로 걸어 본 것은 아니다. **다시 볼 조건**: 다음 시나리오 퀘스트에서
증표를 들었을 때 F8.

### 남은 비용 - 여기서부터는 계획서가 아니라 `PERFORMANCE-PLAN.md` 몫이다
...
혼자**다. 2a 가 떼어 주는 것은 합성기 몫뿐이니, 후보 조언 자체를 줄이는 일(2b·2c)이 다음이다.

**배치 800 ms 는 여전히 사용자가 기다리는 시간이다.** 2단계로 나눠 1.7초에서 800 ms 로 줄었고,
여기서 더 내리는 것은 계획서 3b·5·6·7 의 몫이다.

## 순서와 기대값

| | 하는 것 | 크기 | 위험 | 이 판(4c1efa35)에서 기대하는 것 | 근거 |
|---|---|---|---|---|---|
| **7** | 풀이·게시 계측 | 작음 | 없음 | 전 값이 찍힌다 | 완료·확인됨 |
| **5** | 메모 창 Enter | 아주 작음 | 없음 | 스페이스가 안 보낸다 | 완료·확인됨 |
| **1** | 배치·조언 2단계 게시 | 큼 | 중간 이상 | 배치 게시 warm **631 → 42 ms**(.NET) | 완료·확인됨(밀림 0) |
| **3** | F8 뒤 재계산 생략 | 중간 | 중간 | F8 직후 cold **1,073 ms → ≈ 0** | 완료·확인됨(게시 0.3 ms) |
| **2a** | 합성 추천 거리 켜기 | 작음 | 낮음 | 조언 warm **596 → 156 ms**, 할당 804 → 207 MB | 완료(합성기 층 대기) |
| **4** | 동행 증표 제자리 고정 | 중간 | 중간 | 증표가 있어도 F8 이 된다 | 완료(실기 재현 불가) |
| **6** | 등록부 대조 제거 | 작음 | 낮음 | 폴링 평균 **0.81 → 0.70 ms**(실측) | 완료·확인됨 |
