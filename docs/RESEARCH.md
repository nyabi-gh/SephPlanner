...
단, **닫힌 상자는 거른다.** 상자의 내용물은 스폰 때부터 동기화되어 있어 그대로 읽으면 열기
전에 안이 보인다 — 실제로 열기 전 보상이 추천에 떴다는 사용자 제보로 드러났다. 사람이 알 수
없는 정보를 보여주는 순간 "손으로도 할 수 있는 일의 대행"이라는 원칙이 깨지므로,
`ItemChest.isOpened`(SyncVar)가 참일 때만 후보로 삼는다. 떨어진 꾸러미(`DroppedInventory`)에는
열림 상태가 없는데, 이는 이미 화면에 보이던 인벤토리가 통째로 떨어진 것이라 내용을 숨길 이유가
없어 그대로 둔다.
...
우리 버튼만 줄 오른쪽으로 넘치는데, 창(489)과 줄(382) 사이에 남는 `(489-382)/2 = 53.5` 단위가
우리 몫이다. 그 안에 들어가도록 폭을 좁혀 넣고, 그 자리에 있던 RB 글리프는 그만큼 밀어 준다.

**우리 탭에는 돌기가 없다.** 어느 그림에도 여섯째 자리의 돌기는 없기 때문이다. 그대로 두면
고르는 순간 탭이 사라진 것처럼 구멍이 나므로, 우리 버튼만은 선택돼도 상자 그림을 켜 둔다.
