...
- **이웃 의존 아티팩트.** 효과의 세기가 자기 레벨이 아니라 다른 칸의 내용에 달린 아티팩트들이다.
  ...
  없는 것은 사용자가 강화 우선 지정(우클릭)으로 보정한다. 12종 중 둘을 넣었다.
  ...
  **조화의 수정(`Charm_NearLevelDamage`)도 모델에 넣었다.** 게임 로직은 단순하다:
  ...
  아직 실측 전이다 — 아래 "콤보 가중치" 절의 `--measure`가 함께 재는 대상이다. 다만 **자리
  선택은 환산값과 무관하게 옳다.** 배수가 양수이기만 하면 이웃 레벨 합이 큰 칸을 고른다.
...
레어도다. 사용자가 우클릭으로 강화 우선을 지정하면 그 위에 2배가 곱해진다. 카테고리(콤보)
가치는 배치 점수에 섞지 않고 후보 추천에서만 따로 계산한다.

빔 서치가 배치를 줄 세울 때 쓰는 어림값(`ActiveValue`)만은 여전히 "켜져 있으면 1, 레벨 하나에
1"이다. 어느 아티팩트가 어디 갈지 정하기 전에 배치만 비교하는 값이라 잣대가 같기만 하면 되고,
뽑힌 배치는 뒤에서 실제 값어치로 다시 채점된다.
...
색은 오버레이가 게임 패널에서 이미 채집해 두었으므로(`Theme`) 그 값으로 직접 그린다.
장미빛 테두리 한 겹과 어두운 속 — 오버레이 패널의 골격을 그대로 옮긴 것이다.
...
게임 설정 창에 우리 탭을 넣었다(`OptionsTab`). `UI_OptionsPanel`(UIBase)이 `UI_Tab` 하나를 들고
있고, `UI_Tab`의 `tabButtons`/`tabContents`가 **public 배열**이며 `SelectTab(i)`이 그 색인으로
돈다. `UI_TabButton`과 `UI_TabContent`도 스프라이트 교체와 `activeSelf` 토글이 전부인 단순한
MonoBehaviour 라, 배열에 하나씩 덧붙이면 그만이다. Harmony 패치가 필요 없다.

**만들지 않고 복제한다.** 탭 버튼도 설정 한 줄도 게임 것을 복제해서 쓰므로 생김새·마우스
반응·컨트롤러 이동이 저절로 게임과 같아진다. 우리 일은 복제본에서 게임 쪽 배선을 떼고 우리
값을 물리는 것뿐이다.

**설정 한 줄의 정체는 선택 상자 + 바인더 둘이다.**

- `UI_HorizontalSelectionBox`(Selectable): 좌우로 고르는 상자다. `numberOfElements`,
  `overflowType`, `ChangeValueWithoutNotify(i)`, `OnValueChanged`(C# 이벤트),
  `ValueChangedCallback`(UnityEvent) 이 전부 public 이다. 화살표
  (`UI_HorizontalSelectionBox_Arrow`)는 `parent` 필드로 상자를 가리킨다.
- 바인더(`UI_OptionBox_Common_Integer`, `UI_HorizontalSelectionBox_Text`): 고른 값을 게임 저장
  키(`OptionsBinding`)와 로컬라이즈된 값 글자에 잇는다.

그래서 **바인더를 떼는 것이 우리 값을 물리는 일의 전부다.** 바인더가 들고 있는 `valueText`가
어느 글자가 "값" 표시인지 알려 주므로 이름표와 값을 짐작으로 가려낼 필요도 없다 - 본보기 줄은
바인더가 달린 것으로 고른다.

떼어야 하는 것이 셋이다. `UI_LocalizationStringText`는 **켜질 때마다 제 열쇠로 글자를
덮어쓰므로** 글자만 바꿔 두면 창을 여는 순간 되돌아간다. 바인더는 값을 게임 저장 키에 잇는다.
툴팁 여는 컴포넌트는 원래 줄의 설명을 그대로 띄워 거짓말이 된다. `enabled = false`를 먼저 걸고
Destroy 하는 것은, Destroy 가 프레임 끝에야 도는 사이에 창이 열리면 `OnEnable`이 한 번 돌기
때문이다.

**복제가 안쪽 참조를 다시 이어 준다.** `Instantiate` 는 복제 대상 안을 가리키는 참조를 복제본
쪽으로 바꿔 주므로, 화살표의 `parent` 나 바인더의 `valueText` 는 저절로 복제본의 것이 된다.
바깥을 가리키는 참조(다른 줄로 가는 `forceNav*`)만 원본에 남는데, `Selectable` 이 죽은 참조를
`!= null` 로 걸러 내므로 기본 탐색으로 물러설 뿐이다.

**프리팹에 박힌 UnityEvent 는 지울 수 없고 꺼야 한다.** 복제한 탭 버튼의 `Button.onClick` 은
원본 탭 색인으로 `UI_OptionsPanel.SelectTab` 을 부른다. `RemoveAllListeners` 는 코드로 붙인
것만 지우므로, 인스펙터에서 박은 것은
`SetPersistentListenerState(i, UnityEventCallState.Off)` 로 꺼야 한다. 상자의
`ValueChangedCallback` 은 public 필드라 새 `UnityEvent<int>` 로 통째로 갈아 끼우는 것이 확실하다.

### 탭을 하나 더 붙일 자리는 넉넉하지 않다

**게임 창은 탭 다섯 개에 딱 맞게 짜여 있다.** 에셋에서 뽑은 수치다(단위는 프리팹 좌표).

```
OptionPanel
  Base                      489 x 280          창 그림
    TabArea                 433 x 218
      Tab-GamePlay          420 x 218  ㄱ
      Tab-Sound             420 x 218  │ 탭 내용 다섯. 위쪽 끝이 탭 줄과 같은 높이다
      Tab-Display           420 x 218  │
      Tab-Controls-Keyboard 420 x 218  │
      Tab-Controls-Gamepad  420 x 218  ㄴ
      TabLabel              (가로 늘임, sizeDelta.x = -51) -> 382    탭 줄
        LB / RB             컨트롤러 안내 글리프. 줄 좌우 끝에 붙는다
        ...TabButton x5     각 74 폭, 간격 3
```

`5 x 74 + 4 x 3 = 382` — **다섯 개가 줄 폭을 한 치도 남기지 않고 채운다.** 그래서 하나를 더 넣는
순간 줄이 넘친다. 게다가 `TabLabel`에 `ContentSizeFitter`가 붙어 있어 넘친 만큼 줄 자체가 늘어나고,
가운데 정렬이라 **게임 탭 다섯이 통째로 왼쪽으로 밀린다.**

### 선택된 탭의 흰 돌기는 그림에 박혀 있다

밀리면 왜 안 되는지가 여기 있다. 탭 내용마다 `Image`가 하나씩 있는데, **다섯이 서로 다른
스프라이트를 쓴다**(파일 3의 183 / 208 / 261 / 192 / 166, 전부 `type=0` = Simple). 그림이 곧
그 탭의 프레임이고, **선택된 탭에 붙는 흰 돌기가 그림 안에 이미 그려져 있다.** 게임이 하는 일은
선택된 버튼의 상자 그림을 끄는 것뿐이며(`UI_OptionTabButton.UpdateSprite`), 그 구멍으로 뒤에 있는
내용 그림의 돌기가 드러나는 구조다.

그러니 **버튼을 한 칸이라도 움직이면 게임 탭들이 제 돌기와 어긋난다.** 여섯 개를 줄 폭에 고르게
나눠 보면 어긋남이 최대 58 단위, 화면에서 300px 가까이 된다. 처음에 그렇게 고치려다 실제로
"게임플레이" 탭이 반쯤 가려지는 화면이 나왔다.

**그래서 게임 탭은 손대지 않는다.** `ContentSizeFitter`를 끄고(끄기만 하면 이미 덮어쓴 크기가
남으므로 원래 `sizeDelta`도 되돌린다) 정렬을 왼쪽으로 두면 다섯이 원래 자리에 그대로 선다.
우리 버튼만 줄 오른쪽으로 넘치므로 남는 여백에 들어갈 만큼 좁히고, 그 자리에 있던 RB 글리프는
그만큼 밀어 준다.

**남는 여백은 창 rect 가 아니라 창 그림의 안쪽으로 잰다.** 창 스프라이트 `OptionUi00`(489x280)을
픽셀로 열어 보면 **검은 안쪽이 가운데에서 ±222.5 까지**고 그 바깥은 흙·풀 테두리 그림이다.
rect(±244.5)까지 쓰면 탭이 창 밖 게임 화면 위로 삐져나간다 — 실제로 그렇게 나왔다. 그래서
`222.5 - 191 - 3 = 28.5` 단위가 우리 탭의 폭이고, 게임 탭(74)보다 좁아 이름표가 넘칠 수 있으므로
TMP 자동 크기를 켜서 담는다(들어가면 원래 크기 그대로다).

### 선택된 탭의 돌기는 그림에서 오려낼 수 없다

스프라이트를 열어 보면 구조가 분명하다. `OptionUi_Tab0`(420x218)의 **위 22px 띠에는 x 19~93
구간에만 그림이 있다** — 폭 74, 가운데가 -154 로 첫 탭 버튼과 정확히 겹친다. 색은 흰 테두리
`(236,236,244)` 와 남색 속 `(48,48,70)`. 나머지 네 장은 같은 돌기가 제 탭 자리에 있다.

**탭 상자는 속이 비친 테두리다.** 그래서 선택된 탭의 남색은 상자가 칠하는 것이 아니라 뒤에 있는
내용 그림의 돌기가 비쳐 나오는 것이고, 선택되면 상자 그림을 끄는 것(`UI_OptionTabButton`)만으로
"탭이 내용과 이어진" 모양이 완성된다. 뒤집어 말하면 **우리 내용이 물고 온 원본 자리의 돌기도
그대로 비친다** — 우리 탭을 골랐는데 "게임플레이"가 선택된 것처럼 보인 것이 이것이다.

돌기는 그림에 박혀 있어 옮길 수도 지울 수도 없으므로 **덮고 다시 그린다.** 우리 내용의 자식으로
Image 두 장을 두어, 원본 자리는 창 안쪽 색(검정)으로 덮고 우리 자리에는 같은 남색을 깐다. 둘 다
우리 내용의 자식이라 **우리 탭이 열려 있을 때만 함께 보인다** — 돌기가 보여야 할 때와 정확히 같은
순간이다. 덮는 높이는 버튼(20)이 아니라 돌기(22)를 써야 아래쪽 흰 줄이 남지 않는다.

### 새로 만든 자식은 탭 줄 위에 그려진다

`TabLabel`이 `TabArea`의 **마지막 자식**이라 탭 줄이 탭 내용들보다 뒤에, 즉 위에 그려진다.
`Instantiate` 한 우리 내용은 맨 뒤에 붙으므로 그대로 두면 **탭 줄을 덮는다** — 우리 내용 그림에
박힌 (원본 탭 자리의) 흰 돌기가 첫 탭 버튼을 통째로 가려서, 게임에서 "게임플레이" 탭이 사라진
것처럼 보였다. `SetSiblingIndex`로 탭 줄 바로 앞에 끼우면 게임과 같은 순서가 되고, 엉뚱한 자리의
돌기도 그 탭 버튼 뒤로 숨는다.

**한 줄의 뿌리는 위로 올라가며 찾는다.** 프리팹이 이름표와 상자를 어떻게 묶어 두었는지는 게임
데이터로 알 수 없으므로, 선택 상자에서 부모로 올라가다가 **형제 줄이 함께 들어 있는 곳**을
만나면 거기가 줄들의 그릇이고 그 직전이 한 줄이다. 그릇의 자식을 전부 지우고 우리 줄로 채운다.
본보기를 먼저 복제하고 나서 지워야 한다 - 순서가 뒤집히면 본보기까지 사라진다.

복제할 탭은 **선택 상자가 가장 많은 탭**으로 고른다. 우리 줄이 열이라, 그만큼 담아 본 적이 있는
그릇이어야 넘치지 않는다. 지운 줄 수와 우리 줄 수를 로그에 함께 남긴다.

**이 탭은 입력을 받아도 된다.** HUD 화면의 무입력 원칙은 플레이 중 게임 조작을 방해하지 않기
위한 것인데, 설정 창은 플레이어가 일부러 열어 둔 게임 자신의 창이라 그 이유가 걸리지 않는다.

씬이 바뀌면 설정 창째로 사라지므로 살아 있는지 매 프레임 확인해 다시 붙인다. 붙는 데 실패하면
그 뒤로는 시도하지 않는다 - 같은 예외로 로그를 채우지 않기 위해서다.
...
(우클릭), 콤보 칩 켜고 끄기, 프리셋 코드 붙여넣기, 아이콘/글자 모드 - 은 오버레이에 남아
있다.** 설정에 해당하는 것(켜고 끄기, 자리, 크기, 불투명도, 단축키, 후보 추천 끄기)은 위의
설정 탭으로 갔고, 나머지는 결국 데스크톱 앱 쪽으로 가는 것이 맞아 보인다.

### 아직 확인 못 한 것

**게임을 켜서 봐야 답이 나오는 것들이다.** 후보 추천까지 켠 솔버가 도는 동안 프레임이 실제로
안 끊기는지, 그리고 설정 탭 쪽은 좁힌 탭 버튼(53.5 단위)에 "플래너" 넉 자가 들어가는지다.
`NativeHud`와 `OptionsTab` 모두 무엇을 빌려 왔는지와 못 붙었다면 왜인지를 BepInEx 로그에 남기고,
설정 탭은 붙인 뒤의 좌표를 `%LOCALAPPDATA%\SephPlanner\options-tab.txt`에 그대로 적는다 -
어긋났을 때 스크린샷을 눈으로 재는 대신 숫자를 보기 위해서다.
