...
- **이웃 의존 아티팩트.** 효과의 세기가 자기 레벨이 아니라 다른 칸의 내용에 달린 아티팩트들이다.
  ...
  **넷을 더 넣었다**(`PositionalWorth`, 고정 테스트 `PositionalWorthTests`). 넷 다 공통점이
  있다 — 다른 플래너가 "대상 지정" 버튼로 사람에게 묻는 것들인데, **게임은 자리만 보고 정한다.**
...
**F 키에 해당하는 빈자리가 없다.** 우리는 입력을 가져가지 않으므로(HUD 무입력 보장) 어느 버튼를
...
우리 창처럼 컨트롤 스택에 무엇이 올라가 있는 동안 이 버튼는 게임에서 아무 일도 하지 않는다.
...
새 쪽에는 조이스틱 버튼가 아예 없고, 구식도 버튼 번호가 기기마다 다르다. 지금 무엇으로 놀고
...
**창 안은 이미 컨트롤러로 된다.** 우리 창이 컨트롤 스택에 올라가고(`PlannerPanel`) 버튼가 전부
