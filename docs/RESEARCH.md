...
같은 변경에서 **걸음을 걸음별로 재지 않고 상한까지의 오름폭을 레벨 수로 나눈다.** 예전에는
`step > 0` 인 걸음만 담아서 중간에 안 오르는 레벨이 있는 아티팩트의 환산율이 부풀려졌고,
레벨 상한 위의 닿지 못하는 칸에서도 걸음을 셌다.

결과는 `MOVE_SPEED` 2 → 9.03, `EVASION` 200 → 446 이고, 레벨이 올라도 값어치가 내려가는
아티팩트가 **네 종에서 한 종(도마뱀 판금 갑옷)으로** 줄었다. `Worth.ComboThreshold`는
3.4 → 2.59, `Worth.DamageBonus`는 0.4 → 0.26, `CharmWorth.Tiers`도 새 분위로 다시 뽑았다 —
셋 다 같은 환산율에서 나오므로 함께 움직여야 한다.
