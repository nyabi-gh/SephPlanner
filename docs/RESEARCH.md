...
안 끊기는지, 그리고 설정 탭 쪽은 우리 열 줄이 게임 탭 안에 다 들어가는지 - 본보기로 삼은 탭의
줄 수와 우리 줄 수를 로그에 함께 남기므로 넘치면 거기서 갈린다 - 복제한 탭 버튼에 글자 자리가
있는지, 값 글자 자리가 우리 말("아주 넓게" 등)에 충분한지다. `NativeHud`와 `OptionsTab` 모두
무엇을 빌려 왔는지와 못 붙었다면 왜인지를 BepInEx 로그에 남긴다.
