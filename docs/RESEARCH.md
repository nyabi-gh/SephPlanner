...
오염을 겪은 전례가 있다. 그래서 멀티 세션은 적용을 거부한다. 판정은 `AutoPlacePolicy`(화면
안내와 F8)와 `PlanApplier`(쓰기 직전, 적용 도중 접속이 늘어난 경우까지) 두 겹이다.

한때 호스트 전용 실험 토글(`MultiplayerAutoPlace`)을 두었으나, 개발사가 인벤토리 동기화 구현을
최적화 중이라 바뀔 수 있으니 잠가 두라고 답해 2026-09-03에 뺐다(`docs/LEGAL.md` "받은 답변").
지금 실기 검증을 해도 다음 패치에 무효가 될 수 있어서다. 다시 열려면 개발사의 동기화 작업이
끝난 뒤 호스트 경로부터 검증한다.
...
8. **직전 제안이 앵커다(계획 이력).** `PlanRunner` 가 직전 `Plan` 을 `PlanBuilder` 로 넘기고,
   솔버는 그 배치를 후보로 주입하며 동점일 때 `PlanBonus`(2e-8, 자리 유지 `StabilityBonus`
   1e-6 보다 약하게)로 저번에 말한 자리를 고른다. 우선순위가 중요하다 - 지금 자리 > 직전 제안
...
안 끊기는지, 그리고 설정 창이 화면 가운데에서 적당한 크기인지다. `NativeHud`와 `SettingsWindow`
모두 무엇을 빌려 왔는지와 못 붙었다면 왜인지를 BepInEx 로그에 남긴다.
