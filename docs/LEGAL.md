...
2. **멀티 세션은 기본적으로 읽기 전용이다.** 이유는 약관보다 안정성이다. 선례가 된 커뮤니티
   모드가 멀티 클라이언트에서 석판 회전 미동작, 호스트 인벤토리 오염 등 동기화 문제를 일으켰다.
   클라이언트 쓰기가 Mirror 동기화를 어긋나게 하면 망가지는 것은 공유 세션이고, 그 순간 위
   답변의 단서 조항에 걸린다. 현재 구현에는 `MultiplayerAutoPlace` 실험 토글이 있으나 기본값은
   꺼짐이고 `NetworkServer.active`인 호스트에서만 쓰기를 허용한다. 동기화 실기 검증이 끝날 때까지
   참가자 전원이 동의한 사설 방의 실험 경로로만 취급하며, 클라이언트 쓰기는 계속 거부한다.
...
| 멀티플레이 방해 | 기본값은 쓰기 잠금이다. 명시적 실험 옵션도 참가자 전원이 동의한 사설 방의 호스트로 제한한다 |

두 번째 답변이 **멀티플레이를 금지가 아니라 "참가자 전원 동의" 조건으로 규정한다.** 멀티 자동
배치의 기본 잠금 근거는 약관보다 안정성에 있다(위 2번 참고). 현재 호스트 실험 옵션도 이 조건을
전제로 하며, Mirror 동기화가 안전하다고 확인되기 전에는 정식 지원으로 보지 않는다.
...
- [x] TEAM HORAY에 문의 (2026-08-31 발송, 답 대기)
