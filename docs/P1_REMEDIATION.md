...
### 인수 기록 (2026-09-03, 0.1.2 릴리스 바이너리)

위 "기록할 것" 가운데 이번 세션에서 얻은 것이다. 확인표를 항목별로 짚은 것은 아니고, 기준선을
재는 세션(`docs/ROADMAP.md` "고사양 기준선")에서 로그로 확인된 것만 적는다.

- 게임 버전 `1.0.30`, `Assembly-CSharp` MVID `18f82577233a487384c32357d05323b3`
  (active generation `20260903T105509416-e932f3972fd84ce5a61584dc94c98037`, catalogVersion 7).
- F9 질의 검증: 비교 35616건, 불일치 0건.
- 확인표 첫 항목(정상 싱글 런에서 검증 통과 후 F8 동작): 통과. 로그에 `시뮬레이터 검증 통과 -
  석판 6개와 칸별 레벨까지 일치` 뒤 `자동 배치 완료 - 이동 3건, 회전 0건`.
- 나머지 아홉 항목은 아직 기록이 없다.
...
넣어 후속 P0 작업이 한 곳에서 끝나도록 했다. (후기: 그 P0은 2026-09-03에 개발사 답변에 따라
실험 옵션을 제거하는 것으로 끝났다. 멀티는 예외 없이 잠긴다. `docs/LEGAL.md` "받은 답변".)
