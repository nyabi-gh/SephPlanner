...
**2026-09-13 이전에는 소스가 비공개였고 배포만 `SephPlanner-Release` 저장소로 나갔다.** 그때의
릴리스와 누적 다운로드가 그 저장소에 쌓여 있어서, 합칠 때 소스를 그쪽으로 옮기고 이름을
`SephPlanner`로 바꿨다. 옛 주소는 GitHub 가 리다이렉트한다.
